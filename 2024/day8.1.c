...
int parseInput(FrecPoints antennas[], int *antinodesCount) {
...
void setAntinodes(FrecPoints fp, int *antinodesCount) {
    for (int i = 0; i < fp.pointsLength; i++)
        for (int j = 0; j < fp.pointsLength; j++) {
            ...
            Point distance1 = substruct(fp.points[i], fp.points[j]);
            Point a1 = add(fp.points[i], distance1);
            while (pointInBoundsOfMap(a1)) {
                if (!antinodesMap[a1.y][a1.x]) {
                    (*antinodesCount)++;
            ...
            Point distance2 = substruct(fp.points[j], fp.points[i]);
            Point a2 = add(fp.points[j], distance2);
            while (pointInBoundsOfMap(a2)) {
                if (!antinodesMap[a2.y][a2.x]) {
                    (*antinodesCount)++;
...
int main(void) {
    ...
    int length = parseInput(antennas, &antinodesCount);
